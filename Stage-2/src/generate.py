...
class DPRGeneration(dspy.Signature):
    ...
    - Focus on general insights, comparisons, relationships, or patterns the data should support.
    ...
    data_product_request: str = dspy.OutputField(
        desc="The data product request that captures the information needs or intent across all tables."
    )
...
def generate_dpr_for_cluster(cluster, variant=1, temperature=0.0):
    """Generate a DPR for a single cluster using ChainOfThought."""
    ...
    llm_output = cot(cluster_info=cluster_info)
