...
# override=True: values in .env win over stale GROQ_API_KEY from the shell / OS / IDE (dotenv default is False).
...
# Default for scaled Stage 2 + Stage 1 tables_clean (~100 tables, cluster subsets per DPR).
# Override with env GROQ_MODEL / GROQ_MODEL_LIGHT if needed.
GROQ_MODEL = "llama-3.3-70b-versatile"
# Faster/cheaper model for prose-only steps (mini-summary + final summary). SQL / decompose / judge stay on GROQ_MODEL.
...
class TokensPerDayExhaustedError(RuntimeError):
    """Daily token quota exhausted (e.g. Groq TPD); do not sleep or retry."""
...
def get_llm_client():
    ...
    if groq_key:
        ...
        model_light = os.environ.get("GROQ_MODEL_LIGHT", GROQ_MODEL_LIGHT).strip() or GROQ_MODEL_LIGHT
        return OpenAI(api_key=groq_key, base_url=GROQ_BASE_URL), model, model_light
    raise RuntimeError(
...
def _chat_with_retry(client, model: str, messages: List[Dict[str, str]], temperature: float, max_tokens: int):
    ...
    for attempt in range(MAX_API_RETRIES):
        try:
            ...
            last_exc = e
            err = str(e).lower()
            # TPD: never sleep hours waiting for reset; abort this call so the pipeline can exit fast.
            if "tokens per day" in err or ("tpd" in err and "limit" in err):
...
def _write_stage3_outputs(out: List[Dict[str, Any]], output_path: str, *, partial: bool = False) -> None:
    ...
    prefix = "[stage3] Partial save — tokens-per-day limit. " if partial else ""
    print(
...
def run_stage3_pipeline(
    ...
    client, model, model_light = get_llm_client()
    first = payload[0]
            ...
            out.append(
                {
                    ...
                    "llm_model": model,
                    "llm_model_summaries": model_light,
