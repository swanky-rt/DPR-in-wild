...
PASS 3  Query-relevance enrichment  (display-only, NOT in combined score)
        ...
        Source: --queries_file OR query_text embedded in stage-3_old records.
...
def main():
    ...
    if args.queries_file and os.path.exists(args.queries_file):
        ...
        print("[INFO] No --queries_file — will extract query_text from stage-3_old records")
    ...
    if not all_files:
        ...
        return
    print(f"[INFO] Found {len(all_files)} stage-3_old file(s):")
    for f in all_files:
        print(f"       {f}")
    ...
    for idx, filepath in enumerate(all_files, start=1):
        ...
        # Resolve user_query: queries_file → stage-3_old embedded query_text → none
