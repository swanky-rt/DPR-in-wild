...
- Picks 10 unique tables while limiting how many come from the same domain
- Renames the chosen tables to T1..T10 for consistent local use
...
# --------------------------------------------
#  Select 10 tables with light domain diversity
# --------------------------------------------

def select_diverse_tables(
    dataset,
    num_tables_to_select: int = 10,
...
def main() -> None:
    ...
    # Output locations (relative to this script folder)
    ...
    print("Selecting 10 tables with domain diversity constraint...")
    selected = select_diverse_tables(
        dataset=dataset,
        num_tables_to_select=10,
        max_tables_per_domain=3
    )

    manifest_entries: List[Dict[str, Any]] = []

    # Write each selected table as T1..T10
    for normalized_index, selection in enumerate(selected, start=1):
        ...
        with open(output_table_file_path, "w", encoding="utf-8") as out_f:
            json.dump(
                {
                    "table_id": normalized_table_id,      # normalized ID T1..T10
    ...
    # Print summary
    print("\nDONE.")
    print(f"Wrote 10 table files to: {output_tables_dir}")
