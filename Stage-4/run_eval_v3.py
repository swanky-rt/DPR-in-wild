...
QUALITY (Is the DPR well-formed, analytical, and appropriate in scope for the tables?):
  0    = Factoid/trivial — asks for a simple lookup with no analytical depth
  0.5  = Borderline — some analytical intent but vague or poorly scoped
  0.75 = Analytical — clear goals, reasonable scope, multi-dimensional questions
  1.0  = Excellent — precise, multi-dimensional, measurable goals well-matched to the schema
...
def llm_judge(dpr_text, schema_mapping, api_key="",
              api_base="https://api.openai.com/v1", model="gpt-4") -> dict:
    fallback = {"quality": 0.5,
...
def llm_summary_relevance(
    ...
    """
    Score how relevant the final_summary is to the original DPR text.
    Returns summary_relevance as 0 / 0.5 / 0.75 / 1.0.
    Falls back to 0.5 if no API key or call fails.
    """
    fallback = {"summary_relevance": 0.5,
...
def run_pipeline(raw_data, output_dir, llm_api_key, llm_api_base, llm_model, top_k,
    ...
    for i, d in enumerate(raw_data):
        ...
        summary_judge = llm_summary_relevance(dpr_text, final_summary,
                                              api_key=llm_api_key,
    ...
    # Weights — 7 active terms, sum normalised to 1.0
    # Coverage 0.30 | Complexity 0.20 | Diversity 0.20 | Surprisal 0.20
    # Uniqueness 0.20 | LLM Quality 0.15 | Summary Relevance 0.15
    # LLM Clarity and LLM Relevance (DPR-level) removed.
    _W = 0.30 + 0.20 + 0.20 + 0.20 + 0.20 + 0.15 + 0.15   # = 1.40
    WEIGHTS = {
        "coverage":           round(0.30 / _W, 4),   # 0.2143
        "complexity":         round(0.20 / _W, 4),   # 0.1429
        "diversity":          round(0.20 / _W, 4),   # 0.1429
        "surprisal":          round(0.20 / _W, 4),   # 0.1429
        "uniqueness":         round(0.20 / _W, 4),   # 0.1429
        "llm_quality":        round(0.15 / _W, 4),   # 0.1071
        "summary_relevance":  round(0.15 / _W, 4),   # 0.1071
    }
    ...
    if write_outputs:
        _save_and_print(output, output_dir, ranked)

    return output


def _save_and_print(output, output_dir, ranked=None):
    """Write all output files and print the console ranking table.

    Called by run_pipeline(write_outputs=True) for offline/standalone use.
    Called explicitly by run_eval_all_queries.py AFTER Pass-2 corrections
    so that surprisal/diversity/uniqueness values are final before writing.
    """
    if ranked is None:
        ranked = output["ranked_dprs"]
...
#    1. LLM Relevance score is now printed directly above the Summary block
#    2. Each sub-query block now includes its SQL statement
# ─────────────────────────────────────────────────────────────────────────────
...
def _write_ranking_summary(output: dict, path: str):
    ...
    for r in output["ranked_dprs"]:
        ...
        for label, key in [
            ...
            val = m.get(key)
            if val is not None:
                lines.append(f"  {label:<22} {float(val):.4f}\n")

        lines.append("\n")

        # ── Relevance reasoning block ─────────────────────────────────────────
        # LLM Quality reasoning
        if m.get("llm_reasoning"):
            lines.append("  LLM Reasoning:          "
                         + _wrap(m["llm_reasoning"], width=72, indent=26).lstrip() + "\n")

        # DPR → Summary relevance reasoning
        sum_rel_reason = m.get("summary_relevance_reasoning", "")
        if sum_rel_reason:
            lines.append("  DPR-Summary Reasoning:  "
                         + _wrap(sum_rel_reason, width=72, indent=26).lstrip() + "\n")

        # Query → DPR relevance reasoning
        qdpr_reason = m.get("query_dpr_reasoning", "")
        if qdpr_reason and "No matching user query" not in qdpr_reason:
            lines.append("  Query-DPR Reasoning:    "
                         + _wrap(qdpr_reason, width=72, indent=26).lstrip() + "\n")

        # Query → Summary relevance reasoning
        qsum_reason = m.get("query_summary_reasoning", "")
        if qsum_reason and "No matching user query" not in qsum_reason:
            lines.append("  Query-Summary Reasoning: "
                         + _wrap(qsum_reason, width=72, indent=26).lstrip() + "\n")

        # User query (if enriched)
...
if __name__ == "__main__":
    ...
    if args.sample_k is not None:
        if args.sample_k >= len(raw_data):
            print(f"[INFO] --sample_k {args.sample_k} >= pool size {len(raw_data)}, skipping sampling.")
        else:
            random.seed(42)
            raw_data = random.sample(raw_data, args.sample_k)
            ids = [str(d.get("dpr_id", "?")) for d in raw_data]
            print(f"[INFO] Sampled {args.sample_k} DPRs (seed=42): {ids}")
